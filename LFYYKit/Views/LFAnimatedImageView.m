...
@implementation LFAnimatedImageView {
    BOOL _hasAnimated;
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        _link = [CADisplayLink displayLinkWithTarget:[LFWeakProxy proxyWithTarget:self] selector:@selector(step:)];
...
- (void)setImage:(UIImage *)image {
    ...
    BOOL ani = [image conformsToProtocol:@protocol(LFAnimatedImage)];
    _hasAnimated = (ani && ((UIImage <LFAnimatedImage> *)image).animatedImageCount > 1);
    [self animatedChanged];
}

- (void)setHighlightedImage:(UIImage *)highlightedImage {
    [super setHighlightedImage:highlightedImage];
    BOOL ani = [highlightedImage conformsToProtocol:@protocol(LFAnimatedImage)];
    _hasHighlightAnimated = (ani && ((UIImage <LFAnimatedImage> *)highlightedImage).animatedImageCount > 1);
    [self animatedChanged];
}
...
- (void)animatedChanged {
    ...
    if (!(!_curAnimatedImage && !newImg)) {
        [self resetAnimated];
        _curAnimatedImage = (UIImage <LFAnimatedImage> *)newImg;
        if (newImg) {
            _curFrame = newImg;
...
- (void)calcMaxBufferCount {
    ...
    int64_t total = [UIDevice currentDevice].lf_memoryTotal;
    int64_t free = [UIDevice currentDevice].lf_memoryFree;
    int64_t max = LF_MIN(total * 0.2, free * 0.6);
    max = LF_MAX(max, BUFFER_SIZE);
...
- (void)step:(CADisplayLink *)link {
    UIImage <LFAnimatedImage> *image = _curAnimatedImage;
    ...
    if (_loopEnd) { // view will keep in last frame
        if (_loopEndAction) _loopEndAction();
        [self stopAnimating];
        return;
    }
    ...
    if (!_bufferMiss) {
        ...
        if (_frameChanged) _frameChanged(_curIndex);
    }
