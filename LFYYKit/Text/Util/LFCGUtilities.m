...
//  Created by guoyaoyuan on 15/2/28.
//  
//
...
CGContextRef LFCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef LFCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform LFCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform LFCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    after[0] = [from lf_convertPoint:before[0] toViewOrWindow:to];
    after[1] = [from lf_convertPoint:before[1] toViewOrWindow:to];
    after[2] = [from lf_convertPoint:before[2] toViewOrWindow:to];
    
    return LFCGAffineTransformGetFromPoints(before, after);
}

UIViewContentMode LFCAGravityToUIViewContentMode(NSString *gravity) {
...
NSString *LFUIViewContentModeToCAGravity(UIViewContentMode contentMode) {
    switch (contentMode) {
...
CGRect LFCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode) {
