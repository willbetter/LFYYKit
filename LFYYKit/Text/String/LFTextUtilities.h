...
//  Created by guoyaoyuan on 15/4/6.
//  
//
...
/**
 ...
 @return YES or NO.
 */
static inline BOOL LFTextIsLinebreakChar(unichar c) {
    switch (c) {
        case 0x000D:
        case 0x2028:
        case 0x000A:
...
/**
 ...
 @return YES or NO.
 */
static inline BOOL LFTextIsLinebreakString(NSString *str) {
    if (str.length > 2 || str.length == 0) return NO;
    if (str.length == 1) {
        unichar c = [str characterAtIndex:0];
        return LFTextIsLinebreakChar(c);
    } else {
 ...
 */
static inline NSUInteger LFTextLinebreakTailLength(NSString *str) {
    if (str.length >= 2) {
        unichar c2 = [str characterAtIndex:str.length - 1];
        if (LFTextIsLinebreakChar(c2)) {
        ...
        return LFTextIsLinebreakChar([str characterAtIndex:0]) ? 1 : 0;
    } else {
        return 0;
    }
}
 ...
 */
static inline CGFloat LFEmojiGetAscentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGFloat LFEmojiGetDescentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGRect LFEmojiGetGlyphBoundingRectWithFontSize(CGFloat fontSize) {
    CGRect rect;
    rect.origin.x = 0.75;
    rect.size.width = rect.size.height = LFEmojiGetAscentWithFontSize(fontSize);
    if (fontSize < 16) {
 ...
 */
NSCharacterSet *LFTextVerticalFormRotateCharacterSet();

/**
 Get the character set which should rotate and move in vertical form.
 @return The shared character set.
 */
NSCharacterSet *LFTextVerticalFormRotateAndMoveCharacterSet();
