...
//  Created by guoyaoyuan on 15/3/6.
//  
//
...
@implementation LFTextSimpleMarkdownParser {
    UIFont *_font;
...
- (void)_updateFonts {
    ...
    _boldFont = [_font lf_fontWithBold];
    _italicFont = [_font lf_fontWithItalic];
    _boldItalicFont = [_font lf_fontWithBoldItalic];
...
- (void)setColorWithBrightTheme {
    ...
    _border = [LFTextBorder new];
    _border.lineStyle = LFTextLineStyleSingle;
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    [_regexUnderline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text setTextUnderline:[LFTextDecoration decorationWithStyle:LFTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrikethrough enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text setTextStrikethrough:[LFTextDecoration decorationWithStyle:LFTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexCodeBlock enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (lenStart + lenEnd < r.length) {
            ...
            border.lineStyle = LFTextLineStyleSingle;
...
@implementation LFTextSimpleEmoticonParser {
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        [atr setTextBackedString:[LFTextBackedString stringWithString:subStr] range:NSMakeRange(0, atr.length)];
