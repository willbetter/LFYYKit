...
NSString *const LFTextBackedStringAttributeName = @"LFTextBackedString";
NSString *const LFTextBindingAttributeName = @"LFTextBinding";
NSString *const LFTextShadowAttributeName = @"LFTextShadow";
NSString *const LFTextInnerShadowAttributeName = @"LFTextInnerShadow";
NSString *const LFTextUnderlineAttributeName = @"LFTextUnderline";
NSString *const LFTextStrikethroughAttributeName = @"LFTextStrikethrough";
NSString *const LFTextBorderAttributeName = @"LFTextBorder";
NSString *const LFTextBackgroundBorderAttributeName = @"LFTextBackgroundBorder";
NSString *const LFTextBlockBorderAttributeName = @"LFTextBlockBorder";
NSString *const LFTextAttachmentAttributeName = @"LFTextAttachment";
NSString *const LFTextHighlightAttributeName = @"LFTextHighlight";
NSString *const LFTextGlyphTransformAttributeName = @"LFTextGlyphTransform";

NSString *const LFTextAttachmentToken = @"\uFFFC";
NSString *const LFTextTruncationToken = @"\u2026";


LFTextAttributeType LFTextAttributeGetType(NSString *name){
    if (name.length == 0) return LFTextAttributeTypeNone;
    
    static NSMutableDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = [NSMutableDictionary new];
        NSNumber *All = @(LFTextAttributeTypeUIKit | LFTextAttributeTypeCoreText | LFTextAttributeTypeLFText);
        NSNumber *CoreText_LFText = @(LFTextAttributeTypeCoreText | LFTextAttributeTypeLFText);
        NSNumber *UIKit_LFText = @(LFTextAttributeTypeUIKit | LFTextAttributeTypeLFText);
        NSNumber *UIKit_CoreText = @(LFTextAttributeTypeUIKit | LFTextAttributeTypeCoreText);
        NSNumber *UIKit = @(LFTextAttributeTypeUIKit);
        NSNumber *CoreText = @(LFTextAttributeTypeCoreText);
        NSNumber *LFText = @(LFTextAttributeTypeLFText);
        ...
        dic[(id)kCTStrokeColorAttributeName] = CoreText_LFText;
        dic[NSShadowAttributeName] = UIKit_LFText;
        ...
        dic[(id)kCTGlyphInfoAttributeName] = CoreText_LFText;
        dic[(id)kCTCharacterShapeAttributeName] = CoreText_LFText;
        dic[(id)kCTRunDelegateAttributeName] = CoreText_LFText;
        dic[(id)kCTBaselineClassAttributeName] = CoreText_LFText;
        dic[(id)kCTBaselineInfoAttributeName] = CoreText_LFText;
        dic[(id)kCTBaselineReferenceInfoAttributeName] = CoreText_LFText;
        dic[(id)kCTWritingDirectionAttributeName] = CoreText_LFText;
        ...
        if (kiOS7Later) {
            ...
            dic[(id)kCTLanguageAttributeName] = CoreText_LFText;
        ...
        dic[LFTextBackedStringAttributeName] = LFText;
        dic[LFTextBindingAttributeName] = LFText;
        dic[LFTextShadowAttributeName] = LFText;
        dic[LFTextInnerShadowAttributeName] = LFText;
        dic[LFTextUnderlineAttributeName] = LFText;
        dic[LFTextStrikethroughAttributeName] = LFText;
        dic[LFTextBorderAttributeName] = LFText;
        dic[LFTextBackgroundBorderAttributeName] = LFText;
        dic[LFTextBlockBorderAttributeName] = LFText;
        dic[LFTextAttachmentAttributeName] = LFText;
        dic[LFTextHighlightAttributeName] = LFText;
        dic[LFTextGlyphTransformAttributeName] = LFText;
    });
    NSNumber *num = dic[name];
    if (num) return num.integerValue;
    return LFTextAttributeTypeNone;
}


@implementation LFTextBackedString

+ (instancetype)stringWithString:(NSString *)string {
    LFTextBackedString *one = [self new];
    one.string = string;
    return one;
}
...
@implementation LFTextBinding

+ (instancetype)bindingWithDeleteConfirm:(BOOL)deleteConfirm {
    LFTextBinding *one = [self new];
...
@implementation LFTextDecoration

- (instancetype)init {
    self = [super init];
    _style = LFTextLineStyleSingle;
    return self;
}

+ (instancetype)decorationWithStyle:(LFTextLineStyle)style {
    LFTextDecoration *one = [self new];
    one.style = style;
    return one;
}
...
- (instancetype)init {
    self = [super init];
    self.lineStyle = LFTextLineStyleSingle;
    return self;
}
...
@implementation LFTextAttachment

+ (instancetype)attachmentWithContent:(id)content {
    LFTextAttachment *one = [self new];
    one.content = content;
    return one;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *data = nil;
    @try {
        data = [LFTextArchiver archivedDataWithRootObject:self.attributes];
    }
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    @try {
        _attributes = [LFTextUnarchiver unarchiveObjectWithData:data];
    }
    @catch (NSException *exception) {
#if DEBUG
        NSLog(@"%@",exception);
...
- (void)setShadow:(LFTextShadow *)shadow {
    [self setTextAttribute:LFTextShadowAttributeName value:shadow];
}

- (void)setInnerShadow:(LFTextShadow *)shadow {
    [self setTextAttribute:LFTextInnerShadowAttributeName value:shadow];
}

- (void)setUnderline:(LFTextDecoration *)underline {
    [self setTextAttribute:LFTextUnderlineAttributeName value:underline];
}

- (void)setStrikethrough:(LFTextDecoration *)strikethrough {
    [self setTextAttribute:LFTextStrikethroughAttributeName value:strikethrough];
}

- (void)setBackgroundBorder:(LFTextBorder *)border {
    [self setTextAttribute:LFTextBackgroundBorderAttributeName value:border];
}

- (void)setBorder:(LFTextBorder *)border {
    [self setTextAttribute:LFTextBorderAttributeName value:border];
}

- (void)setAttachment:(LFTextAttachment *)attachment {
    [self setTextAttribute:LFTextAttachmentAttributeName value:attachment];
}
