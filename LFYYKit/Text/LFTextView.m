...
#import "LFTextContainerView.h"
#import "LFTextSelectionView.h"
#import "LFTextMagnifier.h"
#import "LFTextEffectWindow.h"
#import "LFTextKeyboardManager.h"
...
NSString *const LFTextViewTextDidBeginEditingNotification = @"LFTextViewTextDidBeginEditing";
NSString *const LFTextViewTextDidChangeNotification = @"LFTextViewTextDidChange";
NSString *const LFTextViewTextDidEndEditingNotification = @"LFTextViewTextDidEndEditing";


typedef NS_ENUM (NSUInteger, LFTextGrabberDirection) {
    kStart = 1,
    kEnd   = 2,
};

typedef NS_ENUM(NSUInteger, LFTextMoveDirection) {
    kLeft   = 1,
    kTop    = 2,
    kRight  = 3,
    kBottom = 4,
...
@interface _LFTextViewUndoObject : NSObject
@property (nonatomic, strong) NSAttributedString *text;
@property (nonatomic, assign) NSRange selectedRange;
@end
@implementation _LFTextViewUndoObject
+ (instancetype)objectWithText:(NSAttributedString *)text range:(NSRange)range {
    _LFTextViewUndoObject *obj = [self new];
...
@interface LFTextView () <UIScrollViewDelegate, UIAlertViewDelegate, LFTextDebugTarget, LFTextKeyboardObserver> {
    
    LFTextRange *_selectedTextRange; /// nonnull
    LFTextRange *_markedTextRange;
    
    __weak id<LFTextViewDelegate> _outerDelegate;
    ...
    LFTextContainer *_innerContainer; ///< nonnull, inner text container
    LFTextLayout *_innerLayout; ///< inner text layout, the text in this layout is longer than `_innerText` by appending '\n'
    
    LFTextContainerView *_containerView; ///< nonnull
    LFTextSelectionView *_selectionView; ///< nonnull
    LFTextMagnifier *_magnifierCaret; ///< nonnull
    LFTextMagnifier *_magnifierRanged; ///< nonnull
    ...
    LFTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
    LFTextLayout *_highlightLayout; ///< when _state.showingHighlight=YES, this layout should be displayed
    LFTextRange *_trackingRange; ///< the range in _innerLayout, may out of _innerText.
    ...
    struct {
        unsigned int trackingGrabber : 2;       ///< LFTextGrabberDirection, current tracking grabber
        ...
        unsigned int touchMoved : 3;            ///< LFTextMoveDirection, move direction after touch began
...
@synthesize selectedTextRange = _selectedTextRange;  //copy nonnull (LFTextRange*)
@synthesize markedTextRange = _markedTextRange;      //readonly     (LFTextRange*)
...
- (void)_commitUpdate {
    ...
    [[LFTransaction transactionWithTarget:self selector:@selector(_updateIfNeeded)] commit];
}
...
- (void)_updateLayout {
    ...
    [text removeAttribute:LFTextBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:LFTextBackgroundBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    ...
    _innerLayout = [LFTextLayout layoutWithContainer:_innerContainer text:text];
...
- (void)_updateSelectionView {
    ...
    [[LFTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    if (!_innerLayout) return;
    
    NSMutableArray *allRects = [NSMutableArray new];
    BOOL containsDot = NO;
    
    LFTextRange *selectedRange = _selectedTextRange;
    ...
    [allRects enumerateObjectsUsingBlock:^(LFTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
    ...
    if (!_state.firstShowDot && containsDot) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[LFTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
        });
    }
    [[LFTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
...
- (void)_commitPlaceholderUpdate {
    ...
    [[LFTransaction transactionWithTarget:self selector:@selector(_updatePlaceholderIfNeeded)] commit];
}
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        LFTextContainer *container = _innerContainer.copy;
        container.size = self.bounds.size;
        container.truncationType = LFTextTruncationTypeEnd;
        container.truncationToken = nil;
        LFTextLayout *layout = [LFTextLayout layoutWithContainer:_innerContainer text:_placeholderAttributedText];
...
- (void)_updateTextRangeByTrackingCaret {
    ...
    LFTextPosition *newPos = [_innerLayout closestPositionToPoint:trackingPoint];
    if (newPos) {
        ...
        LFTextRange *newRange = [LFTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
...
- (void)_updateTextRangeByTrackingGrabber {
    ...
    LFTextPosition *position = [_innerLayout positionForPoint:magPoint
    ...
    if (position) {
        ...
        if ((NSUInteger)position.offset > _innerText.length) {
            position = [LFTextPosition positionWithOffset:_innerText.length];
        }
        LFTextRange *newRange = [LFTextRange rangeWithStart:(isStart ? position : _selectedTextRange.start)
...
- (void)_updateTextRangeByTrackingPreSelect {
    if (!_state.trackingTouch) return;
    LFTextRange *newRange = [self _getClosestTokenRangeAtPoint:_trackingPoint];
    _trackingRange = newRange;
}
...
- (void)_showMagnifierCaret {
    if (_state.showingMagnifierRanged) {
        ...
        [[LFTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    }
    ...
    if (!_state.showingMagnifierCaret) {
        ...
        [[LFTextEffectWindow sharedWindow] showMagnifier:_magnifierCaret];
    } else {
        [[LFTextEffectWindow sharedWindow] moveMagnifier:_magnifierCaret];
    }
}
...
- (void)_showMagnifierRanged {
    ...
    if (_state.showingMagnifierCaret) {
        ...
        [[LFTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    }
    ...
    if (lineIndex < _innerLayout.lines.count) {
        LFTextLine *line = _innerLayout.lines[lineIndex];
        CGRect lineRect = [self _convertRectFromLayout:line.bounds];
        if (_verticalForm) {
            magPoint.x = LF_CLAMP(magPoint.x, CGRectGetMinX(lineRect), CGRectGetMaxX(lineRect));
        } else {
            magPoint.y = LF_CLAMP(magPoint.y, CGRectGetMinY(lineRect), CGRectGetMaxY(lineRect));
        }
        ...
        if (!_state.showingMagnifierRanged) {
            ...
            [[LFTextEffectWindow sharedWindow] showMagnifier:_magnifierRanged];
        } else {
            [[LFTextEffectWindow sharedWindow] moveMagnifier:_magnifierRanged];
        }
    }
}
...
- (void)_showMenu {
    ...
    if (_selectionView.caretVisible) {
        ...
        LFTextSelectionRect *sRect = _selectionView.selectionRects.firstObject;
        rect = sRect.rect;
        ...
        LFTextKeyboardManager *mgr = [LFTextKeyboardManager defaultManager];
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _highlightLayout = [LFTextLayout layoutWithContainer:_innerContainer text:hiText];
...
- (void)_scrollRangeToVisible:(LFTextRange *)range {
    if (!range) return;
...
- (void)_keyboardChanged {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([LFTextKeyboardManager defaultManager].keyboardVisible) {
            [self _scrollRangeToVisible:_selectedTextRange];
        } else {
...
- (void)_startLongPressTimer {
    ...
    _longPressTimer = [NSTimer timerWithTimeInterval:kLongPressMinimumDuration
                                              target:[LFWeakProxy proxyWithTarget:self]
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if (_highlight.longPressAction) {
            ...
            CGRect rect = [_innerLayout rectForRange:[LFTextRange rangeWithRange:_highlightRange]];
            rect = [self _convertRectFromLayout:rect];
        ...
        if (_state.trackingTouch) {
            if (_state.trackingGrabber) {
                ...
                if (newPos) {
                    ...
                    _trackingRange = [LFTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
                    [self _updateSelectionView];
                }
                [self _hideMenu];
...
- (void)_fixSelectionDot {
    CGPoint origin = [self lf_convertPoint:CGPointZero toViewOrWindow:[LFTextEffectWindow sharedWindow]];
...
- (LFTextRange *)_getClosestTokenRangeAtPosition:(LFTextPosition *)position {
    position = [self _correctedTextPosition:position];
    if (!position) return nil;
    LFTextRange *range = nil;
    if (_tokenizer) {
    ...
    if (range.asRange.length == 0) {
        ...
        LFTextRange *extStart = [_innerLayout textRangeByExtendingPosition:range.start];
        LFTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:range.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            range = [LFTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    range = [self _correctedTextRange:range];
    if (range.asRange.length == 0) {
        range = [LFTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
/// Try to get the character range/position with word granularity from the tokenizer.
- (LFTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point {
    point = [self _convertPointToLayout:point];
    LFTextRange *touchRange = [_innerLayout closestTextRangeAtPoint:point];
    touchRange = [self _correctedTextRange:touchRange];
    
    if (_tokenizer && touchRange) {
        LFTextRange *encEnd = (id)[_tokenizer rangeEnclosingPosition:touchRange.end withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionBackward];
        LFTextRange *encStart = (id)[_tokenizer rangeEnclosingPosition:touchRange.start withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionForward];
        if (encEnd && encStart) {
            NSArray *arr = [@[encEnd.start, encEnd.end, encStart.start, encStart.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [LFTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    if (touchRange) {
        LFTextRange *extStart = [_innerLayout textRangeByExtendingPosition:touchRange.start];
        LFTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:touchRange.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [LFTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    ...
    if (!touchRange) touchRange = [LFTextRange defaultRange];
    
    if (_innerText.length && touchRange.asRange.length == 0) {
        touchRange = [LFTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
- (LFTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    if (!_highlightable || !_innerLayout.containsHighlight) return nil;
    point = [self _convertPointToLayout:point];
    LFTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    LFTextHighlight *highlight = [text attribute:LFTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGFloat)_getMagnifierRangedOffset {
    ...
    LFTextPosition *position = [_innerLayout closestPositionToPoint:magPoint];
    NSUInteger lineIndex = [_innerLayout lineIndexForPosition:position];
    if (lineIndex < _innerLayout.lines.count) {
        LFTextLine *line = _innerLayout.lines[lineIndex];
        if (_verticalForm) {
            magPoint.x = LF_CLAMP(magPoint.x, line.left, line.right);
            return magPoint.x - line.position.x + kMagnifierRangedPopoverOffset;
        } else {
            magPoint.y = LF_CLAMP(magPoint.y, line.top, line.bottom);
...
/// Return a LFTextMoveDirection from `_touchBeganPoint` to `_trackingPoint`.
...
- (BOOL)_isTextPositionValid:(LFTextPosition *)position {
    if (!position) return NO;
    if (position.offset < 0) return NO;
    if (position.offset > _innerText.length) return NO;
    if (position.offset == 0 && position.affinity == LFTextAffinityBackward) return NO;
    if (position.offset == _innerText.length && position.affinity == LFTextAffinityBackward) return NO;
    return YES;
}

/// Whether the range is valid (not out of bounds).
- (BOOL)_isTextRangeValid:(LFTextRange *)range {
...
- (LFTextPosition *)_correctedTextPosition:(LFTextPosition *)position {
    if (!position) return nil;
    ...
    if (position.offset > _innerText.length) {
        return [LFTextPosition positionWithOffset:_innerText.length];
    }
    if (position.offset == 0 && position.affinity == LFTextAffinityBackward) {
        return [LFTextPosition positionWithOffset:position.offset];
    }
    if (position.offset == _innerText.length && position.affinity == LFTextAffinityBackward) {
        return [LFTextPosition positionWithOffset:position.offset];
    }
    return position;
}
...
- (LFTextRange *)_correctedTextRange:(LFTextRange *)range {
    if (!range) return nil;
    if ([self _isTextRangeValid:range]) return range;
    LFTextPosition *start = [self _correctedTextPosition:range.start];
    LFTextPosition *end = [self _correctedTextPosition:range.end];
    return [LFTextRange rangeWithStart:start end:end];
}
...
- (CGPoint)_convertPointToLayout:(CGPoint)point {
    ...
    if (_innerLayout.container.isVerticalForm) {
        ...
        if (boundingSize.width < self.bounds.size.width) {
            if (_textVerticalAlignment == LFTextVerticalAlignmentCenter) {
                point.x += (self.bounds.size.width - boundingSize.width) * 0.5;
            } else if (_textVerticalAlignment == LFTextVerticalAlignmentBottom) {
...
- (void)_replaceRange:(LFTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    if (NSEqualRanges(range.asRange, _selectedTextRange.asRange)) {
        ...
        _selectedTextRange = [LFTextRange rangeWithRange:newRange];
        ...
        if (range.asRange.length != text.length) {
            ...
            if (unionRange.length == 0) {
                ...
                _selectedTextRange = [LFTextRange rangeWithRange:NSMakeRange(range.start.offset + text.length, 0)];
...
- (void)_updateAttributesHolder {
    if (_innerText.length > 0) {
        ...
        [_typingAttributesHolder removeAttribute:LFTextBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:LFTextBackgroundBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
    }
}
...
- (BOOL)_parseText {
    if (self.textParser) {
        LFTextRange *oldTextRange = _selectedTextRange;
        NSRange newRange = _selectedTextRange.asRange;
        ...
        LFTextRange *newTextRange = [LFTextRange rangeWithRange:newRange];
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (_highlightTextAttributes.count) {
                    LFTextHighlight *highlight = [LFTextHighlight highlightWithAttributes:_highlightTextAttributes];
...
- (void)_resetUndoAndRedoStack {
    ...
    _LFTextViewUndoObject *object = [_LFTextViewUndoObject objectWithText:_innerText.copy range:_selectedTextRange.asRange];
...
- (void)_saveToUndoStack {
    ...
    _LFTextViewUndoObject *lastObject = _undoStack.lastObject;
...
- (void)_saveToRedoStack {
    if (!_allowsUndoAndRedo) return;
    _LFTextViewUndoObject *lastObject = _redoStack.lastObject;
    if ([lastObject.text isEqualToAttributedString:self.attributedText]) return;
...
- (BOOL)_canUndo {
    ...
    _LFTextViewUndoObject *object = _undoStack.lastObject;
...
- (void)_redo {
    ...
    _LFTextViewUndoObject *object = _redoStack.lastObject;
...
- (NSArray *)_localizedUndoStrings {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (!strings  && [canonical lf_containsString:@"_"]) {
...
- (void)_setTextParser:(id<LFTextParser>)textParser {
...
- (void)_setLinePositionModifier:(id<LFTextLinePositionModifier>)linePositionModifier {
...
- (void)_initTextView {
    ...
    // UITextInput
    _selectedTextRange = [LFTextRange defaultRange];
    ...
    _innerContainer = [LFTextContainer new];
    ...
    _containerView = [LFTextContainerView new];
    _containerView.userInteractionEnabled = NO;
    _containerView.hostView = self;
    
    _selectionView = [LFTextSelectionView new];
    _selectionView.userInteractionEnabled = NO;
    _selectionView.hostView = self;
    _selectionView.color = [self _defaultTintColor];
    
    _magnifierCaret = [LFTextMagnifier magnifierWithType:LFTextMagnifierTypeCaret];
    _magnifierCaret.hostView = _containerView;
    _magnifierRanged = [LFTextMagnifier magnifierWithType:LFTextMagnifierTypeRanged];
    ...
    self.debugOption = [LFTextDebugOption sharedDebugOption];
    [LFTextDebugOption addDebugTarget:self];
    ...
    [[LFTextKeyboardManager defaultManager] addObserver:self];
...
- (void)dealloc {
    ...
    [[LFTextKeyboardManager defaultManager] removeObserver:self];
    
    [[LFTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    [[LFTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    [[LFTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    
    [LFTextDebugOption removeDebugTarget:self];
...
- (void)scrollRangeToVisible:(NSRange)range {
    LFTextRange *textRange = [LFTextRange rangeWithRange:range];
    textRange = [self _correctedTextRange:textRange];
...
- (void)setText:(NSString *)text {
    ...
    [self replaceRange:[LFTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:text];
}
...
- (void)setTextParser:(id<LFTextParser>)textParser {
    if (_textParser == textParser || [_textParser isEqual:textParser]) return;
    [self _setTextParser:textParser];
    if (textParser && _text.length) {
        [self replaceRange:[LFTextRange rangeWithRange:NSMakeRange(0, _text.length)] withText:_text];
    }
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (text.length == 0) {
        [self replaceRange:[LFTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:@""];
        return;
    }
    ...
    [_inputDelegate textWillChange:self];
     _innerText = text;
    [self _parseText];
    _selectedTextRange = [LFTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    [_inputDelegate textDidChange:self];
    [_inputDelegate selectionDidChange:self];
    ...
    [[NSNotificationCenter defaultCenter] lf_postNotificationOnMainThreadWithName:LFTextViewTextDidChangeNotification object:self];
}

- (void)setTextVerticalAlignment:(LFTextVerticalAlignment)textVerticalAlignment {
...
- (void)setSelectedRange:(NSRange)selectedRange {
    ...
    LFTextRange *range = [LFTextRange rangeWithRange:selectedRange];
    range = [self _correctedTextRange:range];
    [self _endTouchTracking];
...
- (void)setDebugOption:(LFTextDebugOption *)debugOption {
    _containerView.debugOption = debugOption;
}

- (LFTextDebugOption *)debugOption {
    return _containerView.debugOption;
}

- (LFTextLayout *)textLayout {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            LFTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
            ...
            if (_state.trackingCaret) {
                ...
                if (!_state.touchMoved) {
                    if (_state.selectedWithoutEdit) {
                        ...
                        if (self.isFirstResponder) {
                            LFTextRange *_oldRange = _trackingRange;
                            [self _updateTextRangeByTrackingCaret];
...
- (BOOL)becomeFirstResponder {
    ...
    if (!isFirstResponder && become) {
        ...
        [[NSNotificationCenter defaultCenter] lf_postNotificationOnMainThreadWithName:LFTextViewTextDidBeginEditingNotification object:self];
    }
    return become;
}
...
- (BOOL)resignFirstResponder {
    ...
    if (resign) {
        ...
        [[NSNotificationCenter defaultCenter] lf_postNotificationOnMainThreadWithName:LFTextViewTextDidEndEditingNotification object:self];
    }
    return resign;
}
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        UIImage *img = nil;
        if (p.GIFData) {
            img = [LFGIFImage imageWithData:p.GIFData scale:kScreenScale];
        }
        if (!img) {
            img = p.image;
        }
        ...
        if (img && img.size.width > 1 && img.size.height > 1) {
            id content = img;
            if ([img conformsToProtocol:@protocol(LFAnimatedImage)]) {
                id<LFAnimatedImage> ani = (id)img;
                if ([ani animatedImageCount] > 1) {
                    LFAnimatedImageView *aniView = [[LFAnimatedImageView alloc] initWithImage:img];
                    if (aniView) {
                        content = aniView;
                    }
                }
    ...
    if (atr) {
        ...
        LFTextPosition *pos = [self _correctedTextPosition:[LFTextPosition positionWithOffset:endPosition]];
        LFTextRange *range = [_innerLayout textRangeByExtendingPosition:pos];
        range = [self _correctedTextRange:range];
        if (range) {
...
- (void)select:(id)sender {
    ...
    LFTextRange *newRange = [self _getClosestTokenRangeAtPosition:_selectedTextRange.start];
...
- (id<LFTextViewDelegate>)delegate {
    return _outerDelegate;
}

- (void)setDelegate:(id<LFTextViewDelegate>)delegate {
...
#pragma mark - @protocol LFTextKeyboardObserver
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        LFTextBinding *binding = [_innerText attribute:LFTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
        if (binding && binding.deleteConfirm) {
            _state.deleteConfirm = YES;
            [_inputDelegate selectionWillChange:self];
            _selectedTextRange = [LFTextRange rangeWithRange:effectiveRange];
            _selectedTextRange = [self _correctedTextRange:_selectedTextRange];
            [_inputDelegate selectionDidChange:self];
    ...
    _state.deleteConfirm = NO;
    if (range.length == 0) {
        LFTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:_selectedTextRange.end inDirection:UITextLayoutDirectionLeft offset:1];
    ...
    [self replaceRange:[LFTextRange rangeWithRange:range] withText:@""];
}
...
- (void)setSelectedTextRange:(LFTextRange *)selectedTextRange {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        _markedTextRange = [LFTextRange rangeWithRange:NSMakeRange(_selectedTextRange.end.offset, markedText.length)];
        [_innerText replaceCharactersInRange:NSMakeRange(_selectedTextRange.end.offset, 0) withString:markedText];
        _selectedTextRange = [LFTextRange rangeWithRange:NSMakeRange(_selectedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    } else {
        [_innerText replaceCharactersInRange:_markedTextRange.asRange withString:markedText];
        _markedTextRange = [LFTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset, markedText.length)];
        _selectedTextRange = [LFTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    }
...
- (void)replaceRange:(LFTextRange *)range withText:(NSString *)text {
    if (!range) return;
    if (!text) text = @"";
...
- (void)setBaseWritingDirection:(UITextWritingDirection)writingDirection forRange:(LFTextRange *)range {
    if (!range) return;
    range = [self _correctedTextRange:range];
...
- (NSString *)textInRange:(LFTextRange *)range {
    range = [self _correctedTextRange:range];
    if (!range) return @"";
    return [_innerText.string substringWithRange:range.asRange];
}

- (UITextWritingDirection)baseWritingDirectionForPosition:(LFTextPosition *)position inDirection:(UITextStorageDirection)direction {
    [self _updateIfNeeded];
    position = [self _correctedTextPosition:position];
...
- (LFTextPosition *)beginningOfDocument {
    return [LFTextPosition positionWithOffset:0];
}

- (LFTextPosition *)endOfDocument {
    return [LFTextPosition positionWithOffset:_innerText.length];
}

- (LFTextPosition *)positionFromPosition:(LFTextPosition *)position offset:(NSInteger)offset {
    ...
    if (newLocation != 0 && newLocation != _innerText.length) {
        // fix emoji
        [self _updateIfNeeded];
        LFTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:[LFTextPosition positionWithOffset:newLocation]];
    ...
    LFTextPosition *p = [LFTextPosition positionWithOffset:newLocation];
    return [self _correctedTextPosition:p];
}

- (LFTextPosition *)positionFromPosition:(LFTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    [self _updateIfNeeded];
    LFTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:offset];
    ...
    LFTextPosition *newPosition = forward ? range.end : range.start;
    if (newPosition.offset > _innerText.length) {
        newPosition = [LFTextPosition positionWithOffset:_innerText.length affinity:LFTextAffinityBackward];
    }
    
    return [self _correctedTextPosition:newPosition];
}

- (LFTextRange *)textRangeFromPosition:(LFTextPosition *)fromPosition toPosition:(LFTextPosition *)toPosition {
    return [LFTextRange rangeWithStart:fromPosition end:toPosition];
}

- (NSComparisonResult)comparePosition:(LFTextPosition *)position toPosition:(LFTextPosition *)other {
    return [position compare:other];
}

- (NSInteger)offsetFromPosition:(LFTextPosition *)from toPosition:(LFTextPosition *)toPosition {
    return toPosition.offset - from.offset;
}

- (LFTextPosition *)positionWithinRange:(LFTextRange *)range farthestInDirection:(UITextLayoutDirection)direction {
    NSRange nsRange = range.asRange;
    if (direction == UITextLayoutDirectionLeft | direction == UITextLayoutDirectionUp) {
        return [LFTextPosition positionWithOffset:nsRange.location];
    } else {
        return [LFTextPosition positionWithOffset:nsRange.location + nsRange.length affinity:LFTextAffinityBackward];
    }
}

- (LFTextRange *)characterRangeByExtendingPosition:(LFTextPosition *)position inDirection:(UITextLayoutDirection)direction {
    [self _updateIfNeeded];
    LFTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:1];
    return [self _correctedTextRange:range];
}
...
- (LFTextPosition *)closestPositionToPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    LFTextPosition *position = [_innerLayout closestPositionToPoint:point];
    return [self _correctedTextPosition:position];
}

- (LFTextPosition *)closestPositionToPoint:(CGPoint)point withinRange:(LFTextRange *)range {
    LFTextPosition *pos = (id)[self closestPositionToPoint:point];
    if (!pos) return nil;
...
- (LFTextRange *)characterRangeAtPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    LFTextRange *r = [_innerLayout closestTextRangeAtPoint:point];
...
- (NSArray *)selectionRectsForRange:(LFTextRange *)range {
    ...
    [rects enumerateObjectsUsingBlock:^(LFTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
        rect.rect = [self _convertRectFromLayout:rect.rect];
    }];
    return rects;
}
...
- (UITextStorageDirection)selectionAffinity {
    if (_selectedTextRange.end.affinity == LFTextAffinityForward) {
...
- (void)setSelectionAffinity:(UITextStorageDirection)selectionAffinity {
    _selectedTextRange = [LFTextRange rangeWithRange:_selectedTextRange.asRange affinity:selectionAffinity == UITextStorageDirectionForward ? LFTextAffinityForward : LFTextAffinityBackward];
    [self _updateSelectionView];
}

- (NSDictionary *)textStylingAtPosition:(LFTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
...
- (LFTextPosition *)positionWithinRange:(LFTextRange *)range atCharacterOffset:(NSInteger)offset {
    if (!range) return nil;
    ...
    else return [LFTextPosition positionWithOffset:offset];
}

- (NSInteger)characterOffsetOfPosition:(LFTextPosition *)position withinRange:(LFTextRange *)range {
