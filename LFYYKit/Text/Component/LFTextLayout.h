...
@protocol LFTextLinePositionModifier;
extern const CGSize LFTextContainerMaxSize;

/**
 The LFTextContainer class defines a region in which text is laid out.
 LFTextLayout class uses one or more LFTextContainer objects to generate layouts.
 
 A LFTextContainer defines rectangular regions (`size` and `insets`) or 
 ...
 Example:
     ...
     └─────────────────────────────┘
 */
@interface LFTextContainer : NSObject <NSCoding, NSCopying>
...
/// The constrained size. (if the size is larger than LFTextContainerMaxSize, it will be clipped)
...
@property (assign) LFTextTruncationType truncationType;
...
@property (copy) id<LFTextLinePositionModifier> linePositionModifier;
@end


/**
 The LFTextLinePositionModifier protocol declares the required method to modify
 the line position in text layout progress. See `LFTextLinePositionSimpleModifier` for example.
 */
@protocol LFTextLinePositionModifier <NSObject, NSCopying>
@required
/**
 This method will called before layout is completed. The method should be thread-safe.
 @param lines     An array of LFTextLine.
...
/**
 A simple implementation of `LFTextLinePositionModifier`. It can fix each line's position
 to a specified value, lets each line of height be the same.
 */
@interface LFTextLinePositionSimpleModifier : NSObject <LFTextLinePositionModifier>
...
/**
 LFTextLayout class is a readonly class stores text layout result.
 ...
 example: (layout with a circle exclusion path)
     ...
     └──────────────────────────┘
 */
@interface LFTextLayout : NSObject <NSCoding>
...
/**
 ...
 @param containers An array of LFTextContainer object (if nil, returns nil).
 @param text       The text (if nil, returns nil).
 @return An array of LFTextLayout object (the count is same as containers),
...
@property (nonatomic, readonly) LFTextContainer *container;    ///< The text contaner
...
@property (nonatomic, readonly) NSArray *lines;                ///< Array of `LFTextLine`, no truncated
@property (nonatomic, readonly) LFTextLine *truncatedLine;     ///< LFTextLine with truncated token, or nil
@property (nonatomic, readonly) NSArray *attachments;          ///< Array of `LFTextAttachment`
...
/**
 ...
 @return An array of `LFTextSelectionRect` objects that encompass the selection.
...
/**
 ...
 @param range An object representing a range in text.
 @return An array of `LFTextSelectionRect` objects contains the start and end to
...
/**
 ...
 @param debug   The debug option. Pass nil to avoid debug drawing.
 */
- (void)drawInContext:(CGContextRef)context size:(CGSize)size debug:(LFTextDebugOption *)debug;
