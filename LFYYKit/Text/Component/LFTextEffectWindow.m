...
@implementation LFTextEffectWindow

+ (instancetype)sharedWindow {
    static LFTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
...
- (LFTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [LFTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[LFTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return LFTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return LFTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return LFTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return LFTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return LFTextDirectionLeft;
    
    return LFTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(LFTextMagnifier *)mag rotation:(CGFloat)rotation {
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == LFTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == LFTextMagnifierTypeRanged) {
 ...
 */
- (CGFloat)_updateMagnifier:(LFTextMagnifier *)mag {
    UIApplication *app = [UIApplication sharedApplication];
    if (!app) return 0;
    ...
    CGPoint captureCenter = [self lf_convertPoint:mag.hostCaptureCenter fromViewOrWindow:hostView];
    ...
    CGAffineTransform trans = LFCGAffineTransformGetFromViews(hostView, self);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, LFCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(LFTextMagnifier *)mag {
    ...
    CGPoint center = [self lf_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    CGAffineTransform trans = CGAffineTransformMakeRotation(rotation);
    trans = CGAffineTransformScale(trans, 0.3, 0.3);
    mag.transform = trans;
    mag.center = center;
    if (mag.type == LFTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == LFTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)hideMagnifier:(LFTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self lf_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == LFTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(LFSelectionGrabberDot *)dot selection:(LFTextSelectionView *)selection{
    dot.mirror.hidden = YES;
    if (selection.hostView.clipsToBounds == YES && dot.lf_visibleAlpha > 0.1) {
        CGRect dotRect = [dot lf_convertRect:dot.bounds toViewOrWindow:self];
    ...
    CGPoint center = [dot lf_convertPoint:CGPointMake(CGRectGetWidth(dot.frame) / 2, CGRectGetHeight(dot.frame) / 2) toViewOrWindow:self];
