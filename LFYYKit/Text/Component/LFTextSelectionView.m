...
@implementation LFSelectionGrabberDot
...
@implementation LFSelectionGrabber

- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[LFSelectionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}

- (void)setDotDirection:(LFTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == LFTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == LFTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == LFTextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == LFTextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == LFTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == LFTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == LFTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == LFTextDirectionLeft) {
...
@interface LFTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) LFSelectionGrabber *startGrabber;
@property (nonatomic, strong) LFSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation LFTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [LFSelectionGrabber new];
    _startGrabber.dotDirection = LFTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [LFSelectionGrabber new];
    _endGrabber.dotDirection = LFTextDirectionBottom;
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[LFWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? LFTextDirectionRight : LFTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? LFTextDirectionLeft : LFTextDirectionBottom;
    }
}
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(LFTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (LFTextSelectionRect *rect in _selectionRects) {
