...
static int _LFTextKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _LFTextKeyboardViewFrameObserver : NSObject
...
@implementation _LFTextKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_LFTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_LFTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_LFTextKeyboardViewFrameObserverKey);
}
...
@implementation LFTextKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"LFTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _LFTextKeyboardViewFrameObserver *observer = [_LFTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_LFTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static LFTextKeyboardManager *mgr;
...
- (void)_notifyAllObservers {
    ...
    LFTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<LFTextKeyboardObserver> observer in _observers) {
