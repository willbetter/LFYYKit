...
#import "LFLabel.h"
#import "LFAsyncLayer.h"
#import <LFCategory/LFCategory.h>
#import "LFCGUtilities.h"
#import "NSAttributedString+LFText.h"

#if __has_include("LFDispatchQueuePool.h")
#import "LFDispatchQueuePool.h"
#else
#import <libkern/OSAtomic.h>
#endif

#ifdef LFDispatchQueuePool_h
static dispatch_queue_t LFLabelGetReleaseQueue() {
    return LFDispatchQueueGetForQOS(NSQualityOfServiceUtility);
}
#else
static dispatch_queue_t LFLabelGetReleaseQueue() {
...
@interface LFLabel() <LFTextDebugTarget, LFAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText; ///< nonnull
    LFTextLayout *_innerLayout;
    LFTextContainer *_innerContainer; ///< nonnull
    ...
    LFTextLayout *_shrinkInnerLayout;
    LFTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [LFTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [LFLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    LFTextLayout *layout = _innerLayout;
    _innerLayout = nil;
    _shrinkInnerLayout = nil;
    dispatch_async(LFLabelGetReleaseQueue(), ^{
...
- (LFTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (LFTextLayout *)_shrinkLayoutWithLayout:(LFTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        LFTextContainer *container = layout.container.copy;
        container.maximumNumberOfRows = 1;
        CGSize containerSize = container.size;
        if (!container.verticalForm) {
            containerSize.height = LFTextContainerMaxSize.height;
        } else {
            containerSize.width = LFTextContainerMaxSize.width;
        }
        container.size = containerSize;
        return [LFTextLayout layoutWithContainer:container text:layout.text];
    } else {
        return nil;
    }
}
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        LFTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        LFTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
            LFTextPosition *start = [LFTextPosition positionWithOffset:_highlightRange.location];
            LFTextPosition *end = [LFTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:LFTextAffinityBackward];
            LFTextRange *range = [LFTextRange rangeWithStart:start end:end];
...
- (LFTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    LFTextHighlight *highlight = [_innerText attribute:LFTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [LFLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            case LFTextTruncationTypeStart: {
                _lineBreakMode = NSLineBreakByTruncatingHead;
            } break;
            case LFTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case LFTextTruncationTypeMiddle: {
...
- (void)_initLabel {
    ((LFAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [LFTextDebugOption sharedDebugOption];
    [LFTextDebugOption addDebugTarget:self];
    
    _font = [self _defaultFont];
    _textColor = [UIColor blackColor];
    _textVerticalAlignment = LFTextVerticalAlignmentCenter;
    _numberOfLines = 1;
    _lineBreakMode = NSLineBreakByTruncatingTail;
    _innerText = [NSMutableAttributedString new];
    _innerContainer = [LFTextContainer new];
    _innerContainer.truncationType = LFTextTruncationTypeEnd;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if (!_verticalForm && size.width <= 0) size.width = LFTextContainerMaxSize.width;
    if (_verticalForm && size.height <= 0) size.height = LFTextContainerMaxSize.height;
    
    if ((!_verticalForm && size.width == self.bounds.size.width) ||
        (_verticalForm && size.height == self.bounds.size.height)) {
        [self _updateIfNeeded];
        LFTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
    ...
    if (!_verticalForm) {
        size.height = LFTextContainerMaxSize.height;
    } else {
        size.width = LFTextContainerMaxSize.width;
    }
    
    LFTextContainer *container = [_innerContainer copy];
    container.size = size;
    
    LFTextLayout *layout = [LFTextLayout layoutWithContainer:container text:_innerText];
    return layout.textBoundingSize;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    LFTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
    if (innerContainer) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            LFTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (!_state.touchMoved && _textTapAction) {
            ...
            if (!(_ignoreTextTapActionWhenHighlightTapAction && _highlight))
                _textTapAction(self, _innerText, range, rect);
        }
        
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                LFTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    LFTextPosition *start = [LFTextPosition positionWithOffset:_highlightRange.location];
                    LFTextPosition *end = [LFTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:LFTextAffinityBackward];
                    LFTextRange *range = [LFTextRange rangeWithStart:start end:end];
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    switch (lineBreakMode) {
        ...
        case NSLineBreakByCharWrapping:
        case NSLineBreakByClipping: {
            _innerContainer.truncationType = LFTextTruncationTypeNone;
            _innerText.lineBreakMode = lineBreakMode;
        } break;
        case NSLineBreakByTruncatingHead:{
            _innerContainer.truncationType = LFTextTruncationTypeStart;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        case NSLineBreakByTruncatingTail:{
            _innerContainer.truncationType = LFTextTruncationTypeEnd;
        ...
        case NSLineBreakByTruncatingMiddle: {
            _innerContainer.truncationType = LFTextTruncationTypeMiddle;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        default: break;
    }
...
- (void)setTextLayout:(LFTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((LFAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (CGSize)intrinsicContentSize {
    if (_preferredMaxLayoutWidth == 0) {
        LFTextContainer *container = [_innerContainer copy];
        container.size = LFTextContainerMaxSize;
...
#pragma mark - LFAsyncLayerDelegate
...
- (LFAsyncLayerDisplayTask *)newAsyncDisplayTask {
    ...
    LFTextContainer *container = _innerContainer;
    LFTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    LFTextDebugOption *debug = _debugOption;
    ...
    __block LFTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block LFTextLayout *shrinkLayout = nil;
    ...
    // create display task
    LFAsyncLayerDisplayTask *task = [LFAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        LFTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [LFTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [LFLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        if (verticalAlignment == LFTextVerticalAlignmentCenter) {
        ...
        } else if (verticalAlignment == LFTextVerticalAlignmentBottom) {
            if (drawLayout.container.isVerticalForm) {
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (LFTextAttachment *a in drawLayout.attachments) {
        ...
        LFLabel *view = (LFLabel *)layer.delegate;
        if (!view) return;
...
@interface LFLabel(IBInspectableProperties)
@end

@implementation LFLabel (IBInspectableProperties)
...
- (void)setFontName_:(NSString *)fontName {
    ...
    if ((fontName.length == 0 || [fontName.lowercaseString isEqualToString:@"system"]) && ![self fontIsBold_:font]) {
        ...
        if ([self fontIsBold_:font] && ![fontName.lowercaseString lf_containsString:@"bold"]) {
...
- (void)setDebugEnabled_:(BOOL)enabled {
    if (!enabled) {
        ...
        LFTextDebugOption *debugOption = [LFTextDebugOption new];
